...
function ReceiptItem({ r, progress }: { r: ReceiptData; progress: MotionValue<number> }) {
  // Fly in from bottom at 0.50, settle by 0.52
  const y = useTransform(progress, [0.50, 0.52], [r.targetY, 0]);
  const x = useTransform(progress, [0.50, 0.52], [r.targetX, 0]);
  const rot = useTransform(progress, [0.50, 0.52], [r.targetRotate, r.rotate]);
...
export default function ReceiptsBackground({ progress }: Props) {
  return (
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      {receipts.map((r, i) => (
        <ReceiptItem key={i} r={r} progress={progress} />
      ))}
    </div>
  );
}
